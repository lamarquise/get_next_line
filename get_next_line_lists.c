...
/*   Updated: 2019/03/10 19:32:27 by erlazo           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		gnl(char **l, char **s, int fd, t_glst **lst, t_glst *elem)	// keep fd?
{
	...
	free(*s);
	*s = p;
	if (i == 0 && *s == NULL)		// this needs to be thoroughly tested.
	{
		tmp = *lst;
		if (tmp->fd == fd)
		{
	...
	return ((i > 0) ? gnl(l, s, fd, lst, elem) : 0);			// if i were to add another func i would call it here instead of 0 and have it return 0
}																// it would terminate the list elem in question.
